...
class Fusion:
    ...
    def get_parser(self):
        ...
        parser.add_argument('--dump_tsdf', type=bool, default=True, help='Set true to dump TSDF when fuse.')
        ...
        parser.add_argument('--tsdf_dir', type=str, help='Path to dump TSDF directory; files within are overwritten!')
        ...
        parser.add_argument('--vis_tsdf_cut', type=bool, default=False, help='Visualize a cut through the TSDF voxels')
    ...
    def run_fuse(self):
        ...
        for filepath in files:
            ...
            if self.options.dump_tsdf:
                with open(os.path.join(self.options.tsdf_dir, ntpath.basename(filepath)[:-7] + ".npy"), 'wb') as tsdf_f:
                    np.save(tsdf_f, tsdf)

            if self.options.vis_tsdf_cut:
