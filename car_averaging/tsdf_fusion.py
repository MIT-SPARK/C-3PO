...
'''
This is our custom TSDF related library.
'''
class Fusion:
    """
    Performs TSDF fusion.
    Modified from 2_fusion.py script.
    For use in car morphing script
    """
    ...
    def get_parser(self):
        ...
        parser.add_argument('--vis_tsdf_cut', type=bool, default=True, help='Visualize a cut through the TSDF voxels')
    ...
    def get_views(self):
        ...
        for i in range(points.shape[0]):
            ...
            R_y = np.array([[math.cos(longitude), 0, math.sin(longitude)], [0, 1, 0],
    ...
    def get_views_with_points(self, points):
        """
        Generate a set of views to generate depth maps from (based on input points)
        """
        Rs = []
    ...
    def render(self, mesh, Rs):
        ...
        for i in range(len(Rs)):
            ...
            depthmap, mask, img = librender.render(np_vertices.copy(), np_faces.T.copy(), self.render_intrinsics,
    ...
    def get_tsdf(self, files):
        """Get a list of TSDF from input .off files
        """
        tsdf_list = []
        Rs = self.get_views()
        for filepath in files:
    ...
    def load_tsdf(self, files):
        """Load TSDF numpy arrays into a list from files
        """
        tsdf_list = []
        for filepath in files:
    ...
    def tsdf_to_mesh(self, tsdf_list):
        ...
        for tsdf in tsdf_list:
            ...
            vertices -= 0.5
            mesh_list.append({"vertices": vertices, "triangles": triangles})
