...
def chamfer_loss(pc, pc_):
    ...
    #Note: with shape parameter we get peculiar errors!!
...
if __name__ == "__main__":
    ...
    corrector = kp_corrector_pace(cad_models=cad_models, model_keypoints=model_keypoints, weights=weights,
                                  batch_size=batch_size)

    # define pace
    pace = PACEbp(weights=weights, model_keypoints=model_keypoints, batch_size=batch_size)
    ...
    for i, data in enumerate(dataset_loader):
        ...
        R_naive, t_naive, c_naive = pace.forward(y=detected_keypoints)
        _, model_estimate = modelgen.forward(shape=c_naive)
        model_estimate = R_naive @ model_estimate + t_naive
        ...
        _, model_estimate = modelgen.forward(shape=c)
        model_estimate = R @ model_estimate + t
        display_two_pcs(pc1=input_point_cloud.squeeze(0), pc2=model_estimate.squeeze(0))

        # evaluate the two metrics
        print("Evaluation error (wo correction): ", pace_eval(R_naive, rotation_true, t_naive, translation_true,
                                                              c_naive, shape_true))
        print("Evaluation error (w correction): ", pace_eval(R, rotation_true, t, translation_true, c, shape_true))
