...
def generate_shapenet_eval_data(base_folder):
    ...
    for dataset in datasets:
        for object in OBJECT_CATEGORIES:
            ...
            print(f"Generating Dataset: {dataset} ({object})")
            type = dataset.split('.')[1]
            adv_option = dataset.split('.')[2]
            eval_dataset = ShapeNet(type=type,
                                    object=object,
                                    length=50,
                                    num_points=1024,
            ...
            dataset_file = dataset_folder + '/' + object + '.pkl'
            eval_dataset.save_dataset(filename=dataset_file)
...
if __name__ == "__main__":
    ...
    breakpoint()
    generate_shapenet_eval_data(base_folder)
