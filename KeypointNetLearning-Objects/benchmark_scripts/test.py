...
def test(cfg):
    ...
    # model.load_state_dict(torch.load('../../chair/pointnet2/best.pth'))
    ...
    if cfg.task == 'saliency':
        ...
        for data in tqdm(test_dataset):
            ...
            with torch.no_grad():
                logits = model(data)
                # print("logits", logits)
                # print("logits.shape", logits.shape) #torch.Size([1, 2048, 66])
            pred_index = torch.argmax(logits, dim=1)[0].cpu().numpy()
            # print("pred_index", pred_index)
            # print("pred_index.shape", pred_index.shape) #(66,) idx of pcl point
            ...
            geos.append(geo_dists[mesh_fname])
